...
class MeshRefinement(VisualizableClass):
    ...
    def __init__(
        ...
        **kw
        ...
        if self.solvergeom == w3d.RZgeom and 0:
            ...
            if mins is not None:
                mins = [mins[0], 0.0, mins[-1]]
            if maxs is not None:
                maxs = [maxs[0], 0.0, maxs[-1]]
        ...
        if parent is None:
            ...
            if lower is None and upper is None:
                ...
                self.lower = nint(
                    floor((self.mins - self.root.minsglobal) / parent.deltas)
                    * self.refinement
                )
                self.upper = nint(
                    ceil((self.maxs - self.root.minsglobal) / parent.deltas)
                    * self.refinement
                )
            ...
            if npes > 1 and not self.l_EM:
                if self.root.ixproc > 0:
                    self.extradimslower[0] = 1
                if self.root.ixproc < self.root.nxprocs - 1:
                    self.extradimsupper[0] = 1
                if self.root.iyproc > 0:
                    self.extradimslower[1] = 1
                if self.root.iyproc < self.root.nyprocs - 1:
                    self.extradimsupper[1] = 1
                if self.root.izproc > 0:
                    self.extradimslower[2] = 1
                if self.root.izproc < self.root.nzprocs - 1:
                    self.extradimsupper[2] = 1

            rootfulllower = (
                self.root.fulllower * self.totalrefinement
                - self.extradimslower * self.totalrefinement
            )
            rootfullupper = (
                self.root.fullupper * self.totalrefinement
                + self.extradimsupper * self.totalrefinement
            )
            ...
            if fulllower is None:
                self.fulllower = maximum(
                    rootfulllower, self.lower - self.nguard * self.refinement
                )
            else:
                self.fulllower = nint(np.array(fulllower))
            if fullupper is None:
                self.fullupper = minimum(
                    rootfullupper, self.upper + self.nguard * self.refinement
                )
            else:
            ...
            self.fulllower = where(
                self.dims % 2 == 1, self.fulllower - self.refinement, self.fulllower
            )
            ...
            self.fullupper = where(
                self.dims % 2 == 1, self.fullupper + self.refinement, self.fullupper
            )
            ...
            self.deltas = where(
                self.dims > 0.0,
                (self.maxs - self.mins) / where(self.dims > 0.0, self.dims, 1),
                self.deltas,
            )
            ...
            self.bounds[1::2] = where(
                self.fullupper < self.root.dimsglobal * self.totalrefinement,
                interiorbc,
                self.bounds[1::2],
            )
            self.bounds[::2] = where(
                (self.bounds[::2] == 2) & (self.bounds[1::2] != 2),
                interiorbc,
                self.bounds[::2],
            )
            self.bounds[1::2] = where(
            ...
            self.pbounds[1::2] = where(
                self.fullupper < self.root.dimsglobal * self.totalrefinement,
                interiorbc,
                self.pbounds[1::2],
            )
            self.pbounds[::2] = where(
                (self.pbounds[::2] == 2) & (self.pbounds[1::2] != 2),
                interiorbc,
                self.pbounds[::2],
            )
            self.pbounds[1::2] = where(
    ...
    def __setstate__(self, dict):
        ...
        if (
            self.root._unpicklingcount == 0
            and self.lreducedpickle
            and not self.lnorestoreonpickle
        ):
    ...
    def addchild(
        ...
        if self.root.finalized:
            raise RuntimeError(
                "addchild must be called before any operations, such as a field solve, are done"
            )

        if nguard is None:
            nguard = self.nguard
        if nguarddepos is None:
            nguarddepos = self.nguarddepos
    ...
    def installconductor(
        ...
        if "parents" not in self.__dict__:
            return False

        # --- Call the installconductor from the inherited field solver class
        self.__class__.__bases__[1].installconductor(
            self, conductor, xmin, xmax, ymin, ymax, zmin, zmax, dfill
        )
    ...
    def setconductorvoltage(self, voltage, condid=0, discrete=false, setvinject=false):
        ...
        if not self.isfirstcall():
            return
        self.__class__.__bases__[1].setconductorvoltage(
            self, voltage, condid, discrete, setvinject
        )
        for child in self.children:
    ...
    def finalize(self, lforce=0):
        ...
        if self != self.root:
            return
        if self.finalized and not lforce:
            return
    ...
    def initializechilddomains(self):
        ...
        for child in self.children:
            ...
            if self.l_EM:
                l = maximum(
                    self.fulllower, child.fullloweroverrefinement + self.nguarddepos
                )
                ...
                u += where(
                    child.fullupperoverrefinement == self.fullupper,
                    1 + array(self.nguarddepos),
                    0,
                )
                l -= where(
                    child.fullloweroverrefinement == self.fulllower,
                    array(self.nguarddepos),
                    0,
                )
            else:
            ...
            # --- overlap the parent.
            if u[0] < l[0] or u[1] < l[1] or u[2] < l[2]:
                continue
            # --- If the child extends to the edge of the parent mesh, it claims the
            # --- grid points on the upper edges.
            # u = u + where(u == self.fullupper,1,0)
    ...
    def findoverlappingsiblings(
        ...
        if (
            len(blocklists[0]) == 0
            and len(blocklistsxm[0]) == 0
            and len(blocklistsxp[0]) == 0
            and len(blocklistsym[0]) == 0
            and len(blocklistsyp[0]) == 0
            and len(blocklistszm[0]) == 0
            and len(blocklistszp[0]) == 0
        ):
            return
        ...
        for block in blocklists[0]:
            ...
            for sibling in blocklistscopy:
                ...
                if sl[0] > su[0] or sl[1] > su[1] or sl[2] > su[2]:
                    continue
        ...
        for block in blocklists[0]:
            ...
            for neighborlists, pe in zip(neighborblocklists, self.neighborpes):
                if pe < 0 or pe == npes or pe == me:
                    continue
                for neighborblock in neighborlists:
                    if not neighborblock["isactive"]:
                        continue
                    ...
                    if sl[0] > su[0] or sl[1] > su[1] or sl[2] > su[2]:
                        continue
                    if pe < me:
                        block.overlapsparallelleft[pe, neighborblock["blocknumber"]] = [
                            sl,
                            su,
                        ]
                    else:
                        block.overlapsparallelright[
                            pe, neighborblock["blocknumber"]
                        ] = [sl, su]
    ...
    def clearinactiveregions(self, nbcells, parent=None, level=1):
        ...
        for child in self.children:
            ...
            if cl[0] > cu[0] or cl[1] > cu[1] or cl[2] > cu[2]:
                continue
                ...
                for j in range(r[1]):
                    ...
                    for i in range(r[0]):
                        if i == 0:
                            ...
                            isl = slice(-1)
                        nbcstretched[i :: r[0], j :: r[1], k :: r[2]] = nbc[
                            isl, jsl, ksl
                        ]
    ...
    def setsourcepatposition(
        self, x, y, z, ux, uy, uz, gaminv, wfact, zgrid, *args, **kw
    ):
        """
        ...
        if len(self.children) > 0 and not lrootonly:
            ...
            x, y, z, ux, uy, uz, gaminv, wfact, nperchild = self.sortbyichild(
                ichild, x, y, z, ux, uy, uz, gaminv, wfact
            )
        ...
        for block, n in zip(self.root.listofblocks, nperchild):
            if n == 0:
                continue  # does this cause problems? XXX
            self.__class__.__bases__[1].setsourcepatposition(
                block,
                x[i : i + n],
                y[i : i + n],
                z[i : i + n],
    ...
    def exchangesourcepwithneighbors(self):
        ...
        senddictsleft = {}
        senddictsright = {}
        for pe in self.neighborpeslist:
            senddictsleft[pe] = {}
        for pe in self.neighborpeslist:
            senddictsright[pe] = {}
        for block in self.listofblocks:
        ...
        for nel, ner in [[0, 1], [2, 3], [4, 5]]:
            ...
            for parity in [0, 1]:
                if (self.ixproc + self.iyproc + self.izproc) % 2 == parity:
                    if pel in senddictsleft:
                        mpisend(senddictsleft[pel], dest=pel)
                    if per in senddictsright:
                        mpisend(senddictsright[per], dest=per)
                else:
                    if per in senddictsright:
                        dictfromright = mpirecv(source=per)
                    else:
                        dictfromright = {}
                    if pel in senddictsleft:
                        dictfromleft = mpirecv(source=pel)
                    else:
                        dictfromleft = {}
    ...
    def gathersourcepfromchildren(self):
        ...
        for child in self.children:
            ...
            if self.lparallel and self is self.root:
                ...
                fulllower = [
                    ppdecomp.ix[self.ixproc],
                    ppdecomp.iy[self.iyproc],
                    ppdecomp.iz[self.izproc],
                ]
                fullupper = [
                    ppdecomp.ix[self.ixproc] + ppdecomp.nx[self.ixproc],
    ...
    def dosolve(
        self, iwhich=0, zfact=None, isourcepndtscopies=None, indts=None, iselfb=None
    ):
        ...
        # --- value.
        if self.l_internal_dosolve:
            self.setpotentialfromparents()
        self.__class__.__bases__[1].dosolve(
            self, iwhich, zfact, isourcepndtscopies, indts, iselfb
        )
    ...
    def fetchfieldfrompositions(
        self, x, y, z, ex, ey, ez, bx, by, bz, js=0, pgroup=None
    ):
        ...
        self.fetchfieldfrompositionswithoutsort(
            x, y, z, ex, ey, ez, bx, by, bz, js, pgroup
        )
        # self.fetchfieldfrompositionswithpsort(x,y,z,ex,ey,ez,bx,by,bz,js,pgroup)

    def fetchfieldfrompositionswithsort(
        self, x, y, z, ex, ey, ez, bx, by, bz, js=0, pgroup=None
    ):
        """
        ...
        if len(self.children) > 0:
            ...
            particlesortbyindex(
                pgroup, ichild, 0, w3d.ipminfsapi, w3d.npfsapi, nn, nperchild
            )
        ...
        for block, n in zip(self.root.listofblocks, nperchild):
            if n == 0:
                continue
            self.__class__.__bases__[1].fetchfieldfrompositions(
                block,
                x[i : i + n],
                y[i : i + n],
                z[i : i + n],
    ...
    def fetchfieldfrompositionswithoutsort(
        self, x, y, z, ex, ey, ez, bx, by, bz, js=0, pgroup=None
    ):
        """
        ...
        if isort is not None:
            if len(tex) > 0:
                addsortedefield(len(tex), isort, tex, tey, tez, ex, ey, ez)
            if len(tbx) > 0:
                addsortedefield(len(tbx), isort, tbx, tby, tbz, bx, by, bz)
            if pgroup is not None and top.chdtspid > 0:
                # --- Note that these need to be unsorted just like the fields.
                if len(tdx) > 0:
                    addsortedefield(len(tdx), isort, tdx, tdy, tdz, dx, dy, dz)
    ...
    def fetchpotentialfrompositions_old(self, x, y, z, potential):
        ...
        if len(self.children) > 0:
            ...
            for block in [self] + self.children:
                ...
                # --- Now get the field
                if block == self:
                    self.__class__.__bases__[1].fetchpotentialfrompositions(
                        self, xc, yc, zc, tpotential
                    )
                else:
    ...
    def fetchpotentialfrompositionswithoutsort(self, x, y, z, potential):
        ...
        for block, n in zip(self.root.listofblocks, nperchild):
            if n == 0:
                continue
            self.__class__.__bases__[1].fetchpotentialfrompositions(
                block, x[i : i + n], y[i : i + n], z[i : i + n], tpotential[i : i + n]
            )
            i = i + n
        ...
        if isort is not None:
            if len(tpotential) > 0:
                addsortedpotential(len(tpotential), isort, tpotential, potential)
    ...
    def advectchildren(self, zzadvect, nzadvect, root):
        ...
        if self != root:
            ...
            childdomains = self.getchilddomains(
                self.fulllower, self.fullupper, upperedge=1
            )
            if nzadvect > 0:
    ...
    def getwarrayforsourcep(self):
        ...
        for i in range(3):
            if self.dims[i] > 0:
                wi[i] = r[i] - abs(1.0 * iota(-r[i] + 1, +r[i] - 1))
                wi[i] = wi[i] / sum(wi[i])
            else:
                wi[i] = ones(2 * r[i] - 1, "d")
    ...
    def getlocalarray(
        self, localarray, lower, upper, r=[1, 1, 1], fulllower=None, upperedge=1
    ):
        if fulllower is None:
            fulllower = self.fulllower
    ...
    def setmgmaxiters(self, mgmaxiters=None):
        ...
        """
        if mgmaxiters is None:
            mgmaxiters = f3d.mgmaxiters
    ...
    def find_mgparam(self, lsavephi=false, resetpasses=0):
        for block in self.listofblocks:
            ...
            block.children = []
            self.__class__.__bases__[1].find_mgparam(
                block, lsavephi=lsavephi, resetpasses=resetpasses
            )
    ...
    def arraysliceoperation(self, ip, idim, getdataname, op, opnd, null, comp=None):
        ...
        # --- is not active
        if ip < self.fulllower[idim] or ip > self.fullupper[idim] or not self.isactive:
            return null
        ...
        for child in self.children:
            ...
            cresult = child.arraysliceoperation(
                ipc, idim, getdataname, op, opnd, null, ic
            )

            # Ensure that result and cresult are array types.
            result = np.asarray(result, dtype=float).flatten()
            cresult = np.asarray(cresult, dtype=float).flatten()
    ...
    def genericpf(self, getdataname, kw, idim, pffunc, ip=None):
        ...
        if self is not self.root and not self.isactive:
            return
        ...
        if ip is None:
            ...
            if ip is None:
                ip = nint(-self.mins[idim] / self.deltas[idim])
        else:
        ...
        try:
            ...
            if self.fulllower[idim] <= ip and ip <= self.fullupper[idim]:
                if self.childdomains is not None:
                    ...
                    ix, iy, iz = ii
                    ireg[1:, 1:] = equal(
                        self.childdomains[ix, iy, iz], self.blocknumber
                    )
                    if idim != 2:
                        ireg = transpose(ireg)
                    kw["ireg"] = ireg
            ...
            if self is self.root:
                plotlistofthings(lturnofflist=1)
    ...
    def drawbox(self, ip=None, idim=2, withguards=1, color=[], selfonly=0):
        ...
        if self is self.root:
            accumulateplotlists()
        try:
            if ip < self.fulllower[idim] or ip > self.fullupper[idim]:
                return
            ii = [0, 1, 2]
            del ii[idim]
            if withguards:
    ...
    def drawboxzy(self, ix=None, withguards=1, color=[], selfonly=0):
        self.drawbox(
            ip=ix, idim=0, withguards=withguards, color=color, selfonly=selfonly
        )

    def drawboxzx(self, iy=None, withguards=1, color=[], selfonly=0):
        self.drawbox(
            ip=iy, idim=1, withguards=withguards, color=color, selfonly=selfonly
        )

    def drawboxxy(self, iz=None, withguards=1, color=[], selfonly=0):
        self.drawbox(
            ip=iz, idim=2, withguards=withguards, color=color, selfonly=selfonly
        )
    ...
    def plpotentialz(
        self, comp=None, ix=None, iy=None, colors=None, selfonly=0, scale=1
    ):
        if colors is None:
            colors = color
        elif not isinstance(colors, collections.abc.Sequence):
            colors = list([colors])
        if self is self.root:
            accumulateplotlists()
        try:
            if ix < self.fulllower[0]:
                return
            if iy < self.fulllower[1]:
                return
            if ix > self.fullupper[0]:
                return
            if iy > self.fullupper[1]:
                return
            ...
            if comp is None:
                ...
                ppp = self.potential[comp, ...]
            plg(ppp[ix1, iy1, 1:-1], mesh, color=colors[self.blocknumber % len(colors)])
            if not selfonly:
                for child in self.children:
                    child.plpotentialz(
                        comp,
                        ix * child.refinement[0],
                        iy * child.refinement[1],
                        colors=colors,
                        scale=scale,
                    )
        finally:
    ...
    def plpotentialx(
        self, comp=None, iy=None, iz=None, colors=None, selfonly=0, scale=1
    ):
        if colors is None:
            colors = color
        elif not isinstance(colors, collections.abc.Sequence):
        ...
        try:
            if iy < self.fulllower[1]:
                return
            if iz < self.fulllower[2]:
                return
            if iy > self.fullupper[1]:
                return
            if iz > self.fullupper[2]:
                return
    ...
    def plpotentialy(
        self, comp=None, ix=None, iz=None, colors=None, selfonly=0, scale=1
    ):
        if colors is None:
            colors = color
        elif not isinstance(colors, collections.abc.Sequence):
    ...
    def plsourcez(
        ...
        try:
            ...
            if self != self.root and not withboundary:
                if ix == self.fulllower[0] and self.nx > 0:
                    return
                if iy == self.fulllower[1] and self.ny > 0:
                    return
                if ix == self.fullupper[0] and self.nx > 0:
                    return
                if iy == self.fullupper[1] and self.ny > 0:
                    return
                zslice = slice(1, -1)
            else:
                zslice = slice(None)
    ...
    def plsourcex(
        ...
        try:
            ...
            if self != self.root and not withboundary:
                if iy == self.fulllower[1] and self.ny > 0:
                    return
                if iz == self.fulllower[2] and self.nz > 0:
                    return
                if iy == self.fullupper[1] and self.ny > 0:
                    return
                if iz == self.fullupper[2] and self.nz > 0:
                    return
                xslice = slice(1, -1)
            else:
                xslice = slice(None)
    ...
    def plfieldz(
        self, comp=2, ix=None, iy=None, colors=None, selfonly=0, scale=1, withguard=1
    ):
        if colors is None:
            colors = color
        elif not isinstance(colors, collections.abc.Sequence):
        ...
        if withguard:
            ...
            iz = slice(
                self.lower[2] - self.fulllower[2], self.upper[2] - self.fulllower[2] + 1
            )
        if self is self.root:
            accumulateplotlists()
        try:
    ...
    def plfieldx(
        self, comp=2, iy=None, iz=None, colors=None, selfonly=0, scale=1, withguard=1
    ):
        if colors is None:
            colors = color
        elif not isinstance(colors, collections.abc.Sequence):
        ...
        if withguard:
            ...
            lower, upper = self.lower, self.upper
            ix = slice(
                self.lower[0] - self.fulllower[0], self.upper[0] - self.fulllower[0] + 1
            )
        if self is self.root:
            accumulateplotlists()
        try:
    ...
    def plfieldy(
        self, comp=2, ix=None, iz=None, colors=None, selfonly=0, scale=1, withguard=1
    ):
        if colors is None:
            colors = color
        elif not isinstance(colors, collections.abc.Sequence):
        ...
        if withguard:
            ...
            lower, upper = self.lower, self.upper
            iy = slice(
                self.lower[1] - self.fulllower[1], self.upper[1] - self.fulllower[1] + 1
            )
        if self is self.root:
            accumulateplotlists()
        try:
...
class MRBlock3D(MeshRefinement, MultiGrid3D):
    ...
    def plrhoz(
        self, ix=None, iy=None, colors=None, selfonly=0, scale=1, withboundary=0
    ):
        self.plsourcez(
            ix=ix,
            iy=iy,
    ...
    def plrhox(
        self, iy=None, iz=None, colors=None, selfonly=0, scale=1, withboundary=0
    ):
        self.plsourcex(
            iy=iy,
            iz=iz,
    ...
    def plrhoy(
        self, ix=None, iz=None, colors=None, selfonly=0, scale=1, withboundary=0
    ):
        self.plsourcey(
            ix=ix,
            iz=iz,
...
class MRBlock2D(MeshRefinement, MultiGrid2D):
    ...
    def plrhoz(self, ix=None, colors=None, selfonly=0, scale=1, withboundary=0):
        self.plsourcez(
            ix=ix,
            iy=0,
            colors=colors,
    ...
    def plrhox(self, iz=None, colors=None, selfonly=0, scale=1, withboundary=0):
        self.plsourcex(
            iz=iz,
            iy=0,
            colors=colors,
...
class MRBlock2DDielectric(MeshRefinement, MultiGrid2DDielectric):
    ...
    def plepsilonz(self, ix=None, colors=None, selfonly=0, scale=1):
        ...
        try:
            ...
            if scale:
                mesh = self.zmeshlocal[:-1] + self.dz / 2.0
            else:
            ...
            if not selfonly:
                for child in self.children:
                    child.plepsilonz(
                        ix * child.refinement[0], colors=colors, scale=scale
                    )
        finally:
            if self is self.root:
                plotlistofthings(lturnofflist=1)
    ...
    def plepsilonx(self, iz=None, colors=None, selfonly=0, scale=1):
        ...
        try:
            ...
            if not selfonly:
                for child in self.children:
                    child.plepsilonx(
                        iz * child.refinement[2], colors=colors, scale=scale
                    )
        finally:
            if self is self.root:
                plotlistofthings(lturnofflist=1)
...
class MRBlockImplicit2D(MeshRefinement, MultiGridImplicit2D):
    ...
    def plphiz(self, ix=None, colors=None, selfonly=0):
        ...
        try:
            plg(
                self.potential[ix - self.fulllower[0] + 1, 0, 1:-1],
                self.zmesh,
                color=colors[self.blocknumber % len(colors)],
            )
            if not selfonly:
    ...
    def plphix(self, iz=None, colors=None, selfonly=0):
        ...
        try:
            plg(
                self.potential[1:-1, 0, iz - self.fulllower[2] + 1],
                self.xmesh,
                color=colors[self.blocknumber % len(colors)],
            )
            if not selfonly:
    ...
    def plrhoz(self, ix=None, colors=None, selfonly=0):
        ...
        try:
            plg(
                self.source[ix - self.fulllower[0], 0, :],
                self.zmesh,
                color=colors[self.blocknumber % len(colors)],
            )
            if not selfonly:
    ...
    def plrhox(self, iz=None, colors=None, selfonly=0):
        ...
        try:
            plg(
                self.source[:, 0, iz - self.fulllower[2]],
                self.xmesh,
                color=colors[self.blocknumber % len(colors)],
            )
            if not selfonly:
    ...
    def plselfez(self, comp=2, ix=None, colors=None, selfonly=0, withguard=1):
        ...
        try:
            plg(
                self.field[comp, ix - self.fulllower[0], 0, iz],
                self.zmesh[iz],
                color=colors[self.blocknumber % len(colors)],
            )
            if not selfonly:
    ...
    def plselfex(self, comp=2, iz=None, colors=None, selfonly=0, withguard=1):
        ...
        try:
            plg(
                self.field[comp, ix, 0, iz - self.fulllower[2]],
                self.xmesh[ix],
                color=colors[self.blocknumber % len(colors)],
            )
            if not selfonly:
...
class EMMRBlock(MeshRefinement, EM3D):
    ...
    def solve2ndhalf(self):
        ...
        if self.mode == 2:
            ...
            return
        if self.l_verbose:
            print("solve 2nd half", self)
        if top.dt != self.dtinit:
            raise Exception("Time step has been changed since initialization of EM3D.")
        self.push_b_part_2()
        if self.l_pushf:
            self.exchange_f()
        self.exchange_b()
    ...
    def dosolve(self, iwhich=0, *args):
        ...
        if self.solveroff:
            return
        if self.ntsub < 1.0:
            self.novercycle = nint(1.0 / self.ntsub)
        ...
        if self.mode == 2:
            self.dosolvemode2()
            return
        if any(top.fselfb != 0.0):
            raise Exception("Error:EM solver does not work if fselfb != 0.")
        if self.l_verbose:
            print("solve 1st half")
        if top.dt != self.dtinit:
            raise Exception("Time step has been changed since initialization of EM3D.")
        if self.fields.spectral:
            #            self.move_window_fields()
    ...
    def installconductor(
        ...
        result = self.__class__.__bases__[0].installconductor(
            self, conductor, xmin, xmax, ymin, ymax, zmin, zmax, dfill
        )
        if result:
            # --- Call installconductor for the field_coarse
            if self.refinement is not None:
                self.field_coarse.installconductor(
                    conductor, xmin, xmax, ymin, ymax, zmin, zmax, dfill
                )
        return result
    ...
    def addsubstractfieldfromparent(self):
        ...
        for parentnumber in self.parents:
            ...
            if top.efetch[0] != 4:
                addsubstractfields_nodal(
                    self.block,
                    self.field_coarse.block,
                    parent.block,
                    lp,
                    self.refinement,
                    self.l_2dxz,
    ...
    def move_cells_x(self, n):
        ...
        if self.refinement is not None:
            self.__class__.__bases__[1].move_cells_x(
                self.field_coarse, n / self.refinement[0]
            )
        dx = self.dx * n
        self.mins[0] += dx
        self.maxs[0] += dx
    ...
    def move_cells_y(self, n):
        ...
        if self.refinement is not None:
            self.__class__.__bases__[1].move_cells_y(
                self.field_coarse, n / self.refinement[1]
            )
        dy = self.dy * n
        self.mins[1] += dy
        self.maxs[1] += dy
    ...
    def move_cells_z(self, n):
        ...
        if self.refinement is not None:
            self.__class__.__bases__[1].move_cells_z(
                self.field_coarse, n / self.refinement[2]
            )
        dz = self.dz * n
        self.mins[2] += dz
        self.maxs[2] += dz
    ...
    def shift_cells_z(self, n):
        ...
        if self.refinement is not None:
            self.__class__.__bases__[1].shift_cells_z(
                self.field_coarse, n / self.refinement[2]
            )
        dz = self.dz * n
        self.mins[2] += dz
    ...
    def finalize(self, lforce=False):
        ...
        for block in self.listofblocks:
            ...
            if block != self.root:
                self.__class__.__bases__[1].finalize(
                    block.field_coarse, lforce=True
                )  # EM3D.finalize
        self.setbcoverlaps()
    ...
    def finalizesourcep(self):
        if self.l_verbose:
            print("finalizesourcep", self.sourcepfinalized)
        if self.sourcepfinalized:
            return
        ...
        for child in self.children:
            ...
            if 0:
                ...
                apply_dmask(
                    ...
                    cb.core.yf.dmaskz,
                    child.bounds,
                    child.nguarddepos * child.refinement,
                    child.refinement * ntrans,
                    cb.nx,
                    cb.ny,
                    cb.nz,
                    cb.nxguardphi,
    ...
    def fillchilddomains(self):
        if self is not self.root:
            self.__class__.__bases__[1].fillchilddomains(self)
        for child in self.children:
                ...
                for i in range(1, len(self.blocklists)):
                    for c in self.blocklists[i]:
                        if c.isactive:
                            ...
                            if self.l_2dxz and guards:
                                data = data[:, 0, :]
                            slice, dataslice = c.getdataatslice(
                                data, direction, slice, l_abs
                            )
                            if cmin_in is None:
                                cmin = min(cmin, minnd(dataslice))
                            if cmax_in is None:
                                cmax = max(cmax, maxnd(dataslice))
            if cmin_in is None:
                cmin = globalmin(cmin)
            if cmax_in is None:
                cmax = globalmax(cmax)
            gridscale = kw.get("gridscale", None)
            if cmin is not None and gridscale is not None:
                cmin *= gridscale
            if cmax is not None and gridscale is not None:
                cmax *= gridscale
        return slice, cmin, cmax

    def genericpfem3dMR(
        self, title="", dataname="", l_children=1, guards=0, guardMR=0, **kw
    ):
        overlap = True
    ...
    def getarray(self, g, guards=0, overlap=0, guardMR=[0, 0, 0]):
        if guards:
            ...
            ox = oy = oz = 0
            if not overlap:
                if self.block.xrbnd == em3d.otherproc:
                    ox = 1
                if self.block.yrbnd == em3d.otherproc:
                    oy = 1
                if self.block.zrbnd == em3d.otherproc:
                    oz = 1
            if self.l_1dz:
                return g[0, 0, f.nzguard + guardMR[2] : -f.nzguard - oz - guardMR[2]]
            elif self.l_2dxz:
                return g[
                    f.nxguard + guardMR[0] : -f.nxguard - ox - guardMR[0],
                    0,
    ...
    def getdivemrho(self, guards=0, overlap=0, guardMR=[0, 0, 0]):
        ...
        if top.efetch[0] != 4:
            yee2node3d(f)
        return self.getarray(dive, guards, overlap, guardMR) - self.getarray(
            self.fields.Rho / eps0, guards, overlap, guardMR
        )
