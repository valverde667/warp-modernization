...
Test for new parallel 2D dielectric solver with an even number of cells in z.
...
if comm_world.size == 2:
	if z_dependence:
		if comm_world.rank ==0:
			testarray = np.ones([w3d.nx + 2, (w3d.nz + 1) //2 + 3]) * 8.854e-12
			for i in range((w3d.nz+1) // 2 + 3):
				testarray[:,i] = (i+1) * 1.e-11 
			testarray[:,-1] = ((w3d.nz+1) // 2 + 2) * 1.e-11 
		if comm_world.rank ==1:
			testarray = np.ones([w3d.nx + 2, (w3d.nz + 1) //2 + 2]) * 8.854e-12
			for i in range((w3d.nz+1) // 2 + 2):
				testarray[:,i] = (i+5) * 1.e-11 
			testarray[:,0] = ((w3d.nz+1) // 2 + 2) * 1.e-11 
	if not z_dependence:
		pass # No option for this with even		
		#testarray = np.ones([w3d.nx + 2, (w3d.nz+1) // 2 + 2]) * 8.854e-12
