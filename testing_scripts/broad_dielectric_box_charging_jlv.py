...
if l_liveplots:
    def liveplots():
        if top.it%10==0:
            ...
            ppzx(js=0,pgroup=DPart.pgroup,color=cyan,msize=10)
            box.drawzx(color=green)
            refresh()
            os.system('rm -f *.ps *.eps')
...
#winon(2)
#fma();ppg(pscraper.grid.isinside[:,0,:])
#box.isinside([1.e-7],[1.e-7],[8.e-7]).isinside
