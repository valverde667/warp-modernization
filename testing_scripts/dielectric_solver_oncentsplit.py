...
This is an initial test that creates two dielectric regions with a boundary between the dielectrics offset from
...
testarray = np.ones([w3d.nx + 2, w3d.nz + 2]) * 8.854e-12 * 1.5
...
if comm_world.size == 2:
	...
	if z_dependence:
		if comm_world.rank ==0:
			...
			testarray[:,-2:] = 1.5 * 1.e-11 
		if comm_world.rank ==1:
			for i in range((w3d.nz+1) // 2 + 2):
				testarray[:,i] = 1.5 * 1.e-11 
			testarray[:,-4:] = 2 * 1.e-11 
	if not z_dependence:		
		testarray = np.ones([w3d.nx + 2, (w3d.nz+1) // 2 + 2]) * 8.854e-12
	...
	if z_dependence:
		for i in range(4):
			testarray[:,i] = 1. * 1.e-11
		testarray[:,4] = 1.5 * 1.e-11
		for i in range(5,w3d.nz + 2):
